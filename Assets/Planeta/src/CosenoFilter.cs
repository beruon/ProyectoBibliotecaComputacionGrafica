...
public class CosenoFilter: INoiseFilter
{
    Noise noise = new Noise();
    NoiseSettings.RigidNoiseSettings noiseSettings;

    public CosenoFilter(NoiseSettings.RigidNoiseSettings noiseSettings)
    {
    ...
    public float Evaluate (Vector3 point)
    {
        ...
        {
            float v = Mathf.Cos(noise.Evaluate(point * frequency + noiseSettings.center));
            v *= v;
            v *= weight;
