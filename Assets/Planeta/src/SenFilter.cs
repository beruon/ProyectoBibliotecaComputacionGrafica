...
public class SenFilter: INoiseFilter
{
    Noise noise = new Noise();
    NoiseSettings.RigidNoiseSettings noiseSettings;

    public SenFilter(NoiseSettings.RigidNoiseSettings noiseSettings)
    {
    ...
    public float Evaluate (Vector3 point)
    {
        ...
        {
            float v = Mathf.Sin(noise.Evaluate(point * frequency + noiseSettings.center));
            v *= v;
            v *= weight;
